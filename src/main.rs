...
use config::{Config, RiskControllerType, StrategyType};
...
async fn main() -> Result<()> {
    ...
    match &args.command {
        Command::Analytics {
            ...
            let db = match db_manager {
                ...
                None => {
                    return Err(anyhow::anyhow!(
                        "分析模式需要数据库连接，请提供 --db-url 参数或设置 DATABASE_URL 环境变量"
                    ));
                }
            };
            ...
            let range = match time_range.to_lowercase().as_str() {
                ...
                "custom" => {
                    ...
                    if let (Some(start), Some(end)) = (start_date, end_date) {
                        ...
                        return Err(anyhow::anyhow!(
                            "自定义时间范围需要同时提供 --start-date 和 --end-date"
                        ));
                    }
                }
                _ => {
            ...
            match export_format.to_lowercase().as_str() {
                "json" => {
                    let json_path = export_path.join(format!(
                        "report_{}.json",
                ...
                "csv" => {
                    // CSV格式会导出多个文件
                    let report_dir = export_path
                        .join(format!("report_{}", Local::now().format("%Y%m%d_%H%M%S")));
            ...
            println!(
                "平均每笔利润: {:.4} USDT",
                report.overview.avg_profit_per_trade
            );
            println!("=================================\n");
        ...
        _ => {
            ...
            match &args.command {
                ...
                | Command::Simulate {
                    ...
                    // 构造交易对名称
                    config.arbitrage_settings.usdt_symbol =
                        format!("{}{}", args.base_asset, "USDT");
                    config.arbitrage_settings.usdc_symbol =
                        format!("{}{}", args.base_asset, "USDC");
                }
                _ => {}
            }
        }
    ...
    if config.strategy_settings.enabled_strategies.is_empty() {
        return Err(anyhow::anyhow!(
            "未配置任何交易策略，请在配置文件或命令行参数中至少指定一种策略"
        ));
    }
    ...
    info!(
        "最小利润百分比: {}%",
        config.arbitrage_settings.min_profit_percentage
    );
    info!(
        "最大交易金额: {} USDT",
        config.arbitrage_settings.max_trade_amount_usdt
    );
    info!(
        "价格检查间隔: {} ms",
        config.arbitrage_settings.check_interval_ms
    );
    ...
    match args.command {
        Command::Live { .. } => {
            ...
            // 开始监控套利机会
            info!("开始监控 {} - 套利机会", args.base_asset);
        ...
        Command::Simulate {
            ...
            tokio::spawn(async move {
                simulate_price_movements(&api_clone, &base_asset, volatility, opportunity_prob)
                    .await;
            });
...
async fn simulate_price_movements(
    ...
    loop {
        ...
        api.update_price(
            &usdt_symbol,
            Decimal::from_f64(usdt_price).unwrap_or(Decimal::from(50000)),
        );
        api.update_price(
            &usdc_symbol,
            Decimal::from_f64(usdc_price).unwrap_or(Decimal::from(50025)),
        );

        debug!("更新模拟价格 - {usdt_symbol}: {usdt_price:.2}, {usdc_symbol}: {usdc_price:.2}");
