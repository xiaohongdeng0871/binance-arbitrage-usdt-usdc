...
这是一个用Rust编写的多策略自动化套利程序，目前支持USDT和USDC交易对之间的价格差异套利、资金费率套利等策略。程序能够监控市场机会，并在满足设定的利润阈值时自动执行交易。未来还会支持更多套利策略。

## 功能特点

- 实时监控币安交易所的USDT和USDC交易对价格差异
- 支持多种套利策略，包括USDT-USDC价格差异套利和资金费率套利
- 支持自定义套利参数（最小利润百分比、最大交易金额、检查间隔）
- 提供真实交易模式和模拟交易模式
- 自动执行买入和卖出订单，实现套利
- 详细的日志记录和错误处理
- 支持多种基础资产（BTC、ETH等）
- **多种交易策略**：简单价格差异、TWAP、订单簿深度分析、滑点控制、趋势跟踪和资金费率套利（未来还会支持更多套利策略）
- **完善的风控机制**：每日亏损限制、异常价格保护、风险敞口控制、交易时间窗口、交易频率控制和交易对黑名单
- **套利历史记录**：将所有套利交易记录保存到MySQL数据库
- **绩效分析**：生成详细的绩效报告，包括收益统计、成功率分析和币种表现
- **多交易所支持**：目前已支持币安交易所，未来将扩展到更多交易所
...
bash
git clone https://github.com/xiaohongdeng0871/digital-arbitrage.git
cd digital-arbitrage
...
- **funding-rate**: 资金费率套利 - 利用现货和合约市场的资金费率差异进行套利
